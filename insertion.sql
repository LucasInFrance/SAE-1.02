CREATE TABLE CLIENT(
    ...
    CONSTRAINT ck_article_prixu CHECK
        (prixU > 0)
);
CREATE TABLE CLIENT(
    idNumBC DECIMAL(10),
    ...
    CONSTRAINT ck_client_remise CHECK
        (remise >= 0 AND remise <= 100),
        CONSTRAINT ck_client_model
    CHECK
        (
            modeL = 'Do' OR modeL = "RD" OR modeL = 'RR'
        ),
        CONSTRAINT ck_client_typep
    CHECK
        (
            typeP = 'CAB' OR typeP = 'CHB' OR typeP = 'CHD' OR typeP = 'CRD'
        ),
        CONSTRAINT uk_client_numcb UNIQUE(numCB),
        CONSTRAINT ck_client_fraiscr
    CHECK
        (fraisCR = 0.00 OR fraisCR = 6.90),
        CONSTRAINT ck_client_fraisport
    CHECK
        (
            fraisPort = 5.90 OR fraisPort = 6.00 OR fraisPort = 7.50
        ),
